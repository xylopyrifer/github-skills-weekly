...
export function validateDecisions(candidates,document){
 ...
 if(candidate.supporters>=5&&d.status!=='accepted')throw new Error('Five distinct supporters require acceptance');
 if(candidate.supporters<5&&d.status!=='deferred'&&(!Array.isArray(d.sources)||!d.sources.length||!d.sources.every(s=>typeof s==='string'&&s.startsWith('https://github.com/'+candidate.repository+'/')&&s.length<600)))throw new Error('Content reviews require repository source URLs');
 return {repository:candidate.repository,tag:candidate.tag,status:d.status,reason:d.reason,sources:d.sources||[],supporters:candidate.supporters};});
}
export async function run(args=process.argv.slice(2)){
 const community=await readJson('data/community-tags.json',{submissions:[]}),taxonomy=await readJson('config/tags.json'),reviews=await readJson('data/tag-reviews.json',{schema_version:1,decisions:[]});const candidates=reviewCandidates(community.submissions,taxonomy.tags,reviews);
 if(args[0]==='prepare'){const output=args[1]||'.codex/tag-review-queue.json';await writeJson(output,{fingerprint:fingerprint(candidates),candidates});console.log('Review queue: '+output+' ('+candidates.length+' candidates)');return;}
