...
try {
 ...
 await page.route('**/assets/data.json',async route=>{const response=await route.fetch();const data=await response.json();data.weeks=data.weeks.filter(w=>w.start!=='2026-09-07');await route.fulfill({response,json:data});});
