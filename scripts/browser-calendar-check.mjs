...
try {
 ...
 assert.equal(await page.locator('#week-tab-0 small').innerText(),'09.07');
 assert.equal(await page.locator('.rank-card').count(),0);
 assert.match(await page.locator('#week-panel').innerText(),/2026-09-21/);
 ...
 assert.match(await page.locator('#week-tab-0').innerText(),/Last week/);
 assert.match(await page.locator('#week-panel').innerText(),/No complete ranking/);
 assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
 console.log('Calendar regression passed: September period, missing snapshot explanation, historical ranking access, English and mobile layout.');
