...
export async function build(){
  ...
  const publicData={schema_version:1,demo:false,updated_at:catalog.updated_at,total_weeks:history.length,repositories,weeks,all_time:all.repositories.filter(r=>!excluded.has(r.full_name.toLowerCase())).map(({weekly_scores,...r})=>r),warning_count:report.warnings.length};
