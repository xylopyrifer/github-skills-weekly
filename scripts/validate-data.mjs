...
export function validateRecords(catalog,weeks,all,report,{fresh=false,now=new Date()}={}){
 ...
 for(const w of weeks){unique(w.rows);assert.equal(Date.parse(w.end)-Date.parse(w.start),7*86400000,'Invalid week length');assert.equal(new Date(w.start).getUTCDay(),1,'Week must begin Monday');const previous=weeks.find(p=>p.end===w.start);const calculated=weeklyRanking(w.rows,previous?.rows||[]);assert.deepEqual(w.anchors,calculated.anchors,'Scoring anchors mismatch');for(let i=0;i<w.rows.length;i++){const row=w.rows[i],expected=calculated.rows[i];assert.equal(row.full_name,expected.full_name,'Incorrect ranking order');for(const k of ['rank','rank_change','heat_score','components'])assert.deepEqual(row[k],expected[k],'Incorrect '+k+' in '+w.start);if(w.source_kind!=='backfill'){const hours=(Date.parse(row.observed_to)-Date.parse(row.observed_from))/3600000;assert.ok(hours>=162&&hours<=174,'Invalid snapshot comparison window');}else{assert.equal(row.source_kind,'backfill');assert.ok(row.totals_as_of,'Backfill missing provenance');}}}
 assert.deepEqual(all.repositories,aggregateAllTime(weeks),'Cumulative totals mismatch');return {repositories:catalog.repositories.length,weeks:weeks.length,warnings:report.warnings.length};
}
