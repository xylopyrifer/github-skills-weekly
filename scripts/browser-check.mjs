...
try{
 ...
 await page.goto(base);await page.waitForSelector('.search-result');
 ...
 for(let i=0;i<5;i++){
   await page.locator('#week-tab-'+i).click();
   const date=await page.locator('#week-tab-'+i+' small').innerText();
   const available=published.weeks.some(w=>w.start.slice(5).replace('-','.')===date);
   assert.equal(await page.locator('.rank-card').count(),available?10:0);
 }
 await page.locator('#week-tab-0').click();
 results.push('Real data loads with five calendar tabs and explicit data status');
 await page.locator('.search-result').first().click();await page.waitForSelector('.detail-hero');
 ...
 await page.locator('.detail-back').click();await page.waitForSelector('.search-result');
 ...
 const titles=[];
 for(let i=0;i<5;i++){
  await page.locator('#week-tab-'+i).click();assert.equal(await page.locator('.rank-card').count(),10);titles.push(await page.locator('.week-period').innerText());
 }
 assert.equal(new Set(titles).size,5);
 await page.locator('#week-tab-0').click();await page.locator('[data-lang=zh]').click();
 ...
 await page.locator('#week-tab-0').focus();await page.keyboard.press('ArrowRight');assert.equal(await page.locator('#week-tab-1').getAttribute('aria-selected'),'true');
 ...
 await page.setViewportSize({width:390,height:844});await page.locator('#week-tab-0').click();await page.screenshot({path:'release/qa/mobile-demo.png',fullPage:true});
 results.push('Homepage and detail have no horizontal overflow at 320, 375, 390 and 768 pixels');
 await page.locator('[data-action=live]').click();await page.waitForSelector('.search-result');assert.equal(await page.locator('.notice.demo').count(),0);
 ...
 await failed.unroute('**/assets/data.json');await failed.locator('[data-action=retry]').click();await failed.waitForSelector('.search-result');
 results.push('Network error state recovers using Retry');
 const denied=await browser.newPage();await denied.addInitScript(()=>Object.defineProperty(window,'localStorage',{get(){throw new Error('storage denied');}}));await denied.goto(base);await denied.waitForSelector('.search-result');await denied.locator('[data-lang=en]').click();assert.equal(await denied.locator('html').getAttribute('lang'),'en');
