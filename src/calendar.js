const DAY=86400000;
export function completedWeekStart(now=new Date()){const d=new Date(now);d.setUTCHours(0,0,0,0);d.setUTCDate(d.getUTCDate()-(d.getUTCDay()+6)%7-7);return d.toISOString().slice(0,10);}
export function calendarSlots(data,now=new Date()){
 const anchor=data.demo&&data.weeks.length?data.weeks.map(w=>w.start).sort().at(-1):completedWeekStart(now);
