...
const t=k=>messages[language][k]||k;
...
function renderHome(){
 ...
 if(week)c+=`<div class="week-period"><span>${period(week)}</span><span>UTC · ${Math.min(10,week.rows.length)} SKILLS</span></div><div class="rank-list">${week.rows.slice(0,10).map(row=>repoCard(repo(row.full_name)||row,row)).join('')}</div><p class="small-note">${t('periodNote')}</p>`;
...
function renderDetail(name){
 ...
 const history=slots.slice().reverse().map(slot=>{const p=slot.week&&slot.week.rows.find(w=>w.full_name.toLowerCase()===r.full_name.toLowerCase());return`<div class="trend-row"><div class="trend-label">${slot.start.slice(5).replace('-','.')}<small>${p?'#'+p.rank:t('missing')}</small></div><div class="trend-track"><div class="trend-fill" style="width:${p?p.heat_score:0}%"></div></div><span class="trend-value">${p?fmt(p.heat_score):'—'}</span></div>`;}).join('');
 main.innerHTML=`<a class="detail-back" href="#">← ${t('back')}</a>${notice()}<article><div class="detail-hero"><div class="source-label">${t(row?'historicalData':'currentData')}${row?' · '+esc(week.week):''}</div><div class="detail-title-row">${icon(r.full_name)}<div><h1>${esc(r.name||repoName(r.full_name))}</h1><p>${esc(r.full_name)}</p></div></div><p class="detail-description">${esc(description(r))}</p><div class="detail-actions">${!demo?`<a class="primary-button" href="https://github.com/${esc(r.full_name)}" target="_blank" rel="noopener noreferrer">${t('github')} ↗</a>`:`<span class="period-chip">${t('sampleRepo')}</span>`}<span class="period-chip">${esc(r.language||t('unknown'))}</span></div><div class="detail-stats">${stat('heat',row?fmt(row.heat_score):'—',true)}${stat('stars',fmt(s.stars))}${stat('starGrowth',row?signed(row.stars_delta):'—',true)}${stat('forks',fmt(s.forks))}${stat('forkGrowth',row?signed(row.forks_delta):'—',true)}</div></div><div class="detail-grid"><div><section class="panel"><div class="section-heading"><h2>${t('history')}</h2><span class="subtle">${t('historySub')}</span></div><div class="trend">${history}</div><p class="small-note">${t('periodNote')}</p></section><section class="panel"><h2>${t('why')}</h2><ul>${highlights.map(h=>`<li>${esc(h)}</li>`).join('')}</ul><h2 style="margin-top:25px">${t('audience')}</h2><p>${esc(audience)}</p></section>${r.description?`<section class="panel raw-description"><h2>${t('repoDescription')}</h2><p>${esc(r.description)}</p><small class="subtle">${t('original')}</small></section>`:''}</div><aside><section class="panel"><h2>${t('alltime')}</h2>${pair('alltimeRank',all?'#'+all.current_rank:'—')}${pair('totalHeat',all?fmt(all.total_heat):'—')}${pair('appearances',all?fmt(all.weeks_ranked):'0')}${pair('bestRank',all?'#'+all.best_rank:'—')}${pair('streak',all?all.current_streak+' '+t('weeks'):'—')}</section><section class="panel"><h2>${t('evidence')}</h2><div class="evidence-list">${evidence}${r.manual?`<span>${t('manual')}</span>`:''}</div><p class="small-note">${t('relevance')} ${r.relevance?fmt(r.relevance.score):'—'}/100</p></section><section class="panel"><h2>${t('scoreParts')}</h2>${pair('methodStars','60%'+(row?' · '+row.components.stars.toFixed(1):''))}${pair('methodForks','15%'+(row?' · '+row.components.forks.toFixed(1):''))}${pair('methodActivity','15%'+(row?' · '+row.components.activity.toFixed(1):''))}${pair('methodSize','10%'+(row?' · '+row.components.size.toFixed(1):''))}<p class="small-note">${t('componentsNote')}</p></section></aside></div><p class="source-time">${t('snapshot')}: ${dateTime(r.last_updated)}</p>${row?`<p class="source-time">${t('observed')}: ${dateTime(row.observed_from)} → ${dateTime(row.observed_to)}</p>`:''}${row&&row.activity_capped?`<p class="small-note">${t('activityCapped')}</p>`:''}</article>`;
 document.title=(r.name||repoName(r.full_name))+' — GitHub Skills Weekly';
}
function footer(){document.getElementById('footer').innerHTML=`<div><strong>Skills Weekly</strong><p>${t('source')} · ${t('updated')}: ${dateTime(data&&data.updated_at)}</p><p>${t('freshness')}</p></div><div class="footer-right"><p>${t('privacy')}</p><p>© 2026 <a href="https://github.com/xylopyrifer" target="_blank" rel="noopener noreferrer">xylopyrifer</a> · <a href="./LICENSE" target="_blank" rel="noopener">MIT</a></p><p>${t('attribution')}</p>${!demo&&data&&data.weeks.length?`<button type="button" class="text-button" data-action="demo">${t('sample')}</button>`:''}</div>`;}
function render(){
